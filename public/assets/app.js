...
const sourceLinks = arr => (arr || []).map(s => `<a href="${esc(s.url)}" target="_blank" rel="noopener">${esc(s.name)} ↗</a>`).join(' &nbsp; ');
const spark = values => {
...
function render(d){
  ...
  <section id="regime" class="section">
      ...
      <article class="card regime-card">
        ...
        <div class="regime-body">
          ...
          <div class="gauge">
            ...
            <div class="value">${r.breadth_score > 0 ? '+' : ''}${esc(r.breadth_score)}</div><small>${esc(r.breadth_note)}</small>
          </div>
        </div>
    ...
    <div class="card ticker-strip">${(d.market_tiles||[]).map(t=>`<div class="ticker"><div class="label">${esc(t.label)}</div><div class="big ${t.direction==='down'?'down':''}">${esc(t.value)}</div><div class="spark">${spark(t.spark)}</div></div>`).join('')}</div>
  </section>
  ...
  <section id="macro" class="section">
    ...
    <div class="macro-grid">
      <article class="card macro-main"><div class="macro-stat"><div class="eyebrow" style="color:#79c8ff">${esc(d.macro_rates.lead_label)}</div><div><div class="big">${esc(d.macro_rates.lead_value)}</div><small>${esc(d.macro_rates.lead_delta)}</small></div></div>
      ...
      <aside class="card timeline"><h3>Prossimi catalyst</h3>${(d.catalysts||[]).map(c=>`<div class="timeline-item"><b>${esc(c.date)}</b><span class="dot"></span><div><strong>${esc(c.title)}</strong><br><small>${esc(c.summary)}</small></div></div>`).join('')}</aside>
    </div>
  </section>
  ...
  <section id="intermarket" class="section">
    ...
    <div class="card market-summary"><div><div class="eyebrow" style="color:var(--amber)">WALL STREET · INDICI CASH</div><h3>${esc(d.indices_summary.us)}</h3></div><div><div class="eyebrow" style="color:var(--amber)">EUROPA · INDICI CASH</div><h3>${esc(d.indices_summary.europe)}</h3></div></div>
    <div class="card table"><div class="table-header"><div>Indice</div><div>Livello / stato</div><div>Variazione</div></div>${(d.indices||[]).map(x=>`<div class="table-row"><div class="name">${esc(x.name)}</div><div>${esc(x.level)}</div><div class="${cls(x.direction)}">${esc(x.change)}</div></div>`).join('')}</div>
    <div class="card inter-list">${(d.intermarket||[]).map((x,i)=>`<div class="inter-row"><div class="idx">${String(i+1).padStart(2,'0')}</div><div><div class="instrument">${esc(x.name)}</div><div class="desc">${esc(x.description)}</div></div><div class="value">${esc(x.value)}</div><div class="${cls(x.direction)}">${esc(x.change)}</div></div>`).join('')}</div>
  </section>
  ...
  <section id="earnings" class="section">
    ...
    <div class="appointments"><b style="color:var(--amber);letter-spacing:.15em;margin-right:20px">PROSSIMI APPUNTAMENTI</b>${(d.appointments||[]).map(esc).join(' · ')}</div>
  </section>
  ...
  <section id="movers" class="section">
    ...
    <div class="card table"><div class="table-header"><div>Ticker</div><div>Movimento</div><div>Catalyst</div></div>${(d.key_movers||[]).map(x=>`<div class="table-row"><div class="name">${esc(x.ticker)}</div><div class="${cls(x.direction)}">${esc(x.change)}</div><div>${esc(x.catalyst)}</div></div>`).join('')}</div>
  </section>
